...
class DoubleDownPhase9Tests(unittest.TestCase):
    ...
    def test_create_persists_ready_challenge(self):
        snapshot = self.service.create(starting_balance=Decimal("100"), failure_floor=Decimal("20"))
    ...
    def test_start_pause_resume_and_terminate_are_persisted(self):
        created = self.service.create(starting_balance=Decimal("100"), failure_floor=Decimal("20"))
        ...
        self.assertEqual(self.persistence.snapshots[str(challenge_id)]["state"]["status"], "terminated")
