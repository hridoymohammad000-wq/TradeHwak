...
export interface MarketTicker {
  ...
  low24h: number;
  volume24h: number;
  sparkline: number[]; // Array of last 10 prices for visual trend rendering
}
...
// Scanner / Signals canonical backend types
...
export interface ScannerResult {
  ...
  outcome: ScanOutcome;
  mode: TradingMode;
  timeframe: BackendTimeframe | null;
  direction: BackendDirection | null;
...
export interface CanonicalScanData {
  mode: TradingMode;
  timeframe: BackendTimeframe | null;
  counts: ScanCounts;
...
export interface CanonicalSignalsData {
  filters: {
    mode: TradingMode;
    grade: BackendGrade | null;
    symbol: string | null;
...
// Canonical Chart Workspace types
...
// Canonical Control Center / Settings types
...
export interface ControlCenterSettings {
 ...
 risk:{daily_max_loss:number; daily_max_trades:number; risk_per_trade_pct:number; max_open_positions:number; scalping:ModeRiskSettings; intraday:ModeRiskSettings};
 notifications:{telegram:boolean; email:boolean; chime:boolean; toast:boolean};
...
// Canonical FastAPI data contracts used by Dashboard, Active Trades, and Journal.
...
export interface CanonicalActiveTrade {
  trade_id: string;
  order_id: string | null;
  signal_id: string | null;
  symbol: string;
  mode: CanonicalTradingMode;
  direction: CanonicalTradeDirection;
  qty: string | null;
  entry_price: number | null;
  current_price: number | null;
...
export interface CanonicalClosedTrade {
  ...
  pnl_multiple_of_risk: number | null;
  stop_slippage_usdt: number | null;
  risk_reward: number | null;
  result: string | null;
  status: string;
...
export interface CanonicalTodaySummary {
...
export interface CanonicalActiveTradesData {
  today_summary: CanonicalTodaySummary;
  active_trades: CanonicalActiveTrade[];
  scalping_trades: CanonicalActiveTrade[];
  intraday_trades: CanonicalActiveTrade[];
  unknown_trades: CanonicalActiveTrade[];
  range_start: string | null;
  range_end: string | null;
}

export interface JournalSummaryMetric {
  total_trades: number;
  wins: number;
  losses: number;
...
export interface JournalSummaries {
...
export interface CanonicalJournalData {
  closed_trades: CanonicalClosedTrade[];
...
export interface CanonicalDashboardData {
  ...
  system_mode: 'demo';
  active_strategy_mode: 'scalping' | 'intraday';
  scalping_engine_enabled: boolean;
  intraday_engine_enabled: boolean;
  auto_trade_enabled: boolean;
  emergency_stop: boolean;
  account: DashboardAccountSummary;
  today_summary: CanonicalDashboardTodaySummary;
  recent_events: DashboardEventRecord[];
  range_start: string | null;
  range_end: string | null;
}
...
export interface PerformanceSummaryMetric {
  ...
  realized_pnl: number | null; average_realized_pnl: number | null; win_rate: number | null;
  average_risk_reward: number | null; best_trade: number | null; worst_trade: number | null;
  stop_loss_hit_count: number; take_profit_hit_count: number; manual_close_count: number; emergency_stop_count: number;
}
export interface PerformanceAnalysisData {
  trades: CanonicalClosedTrade[];
  summaries: { scalping: PerformanceSummaryMetric; intraday: PerformanceSummaryMetric; unknown: PerformanceSummaryMetric; combined: PerformanceSummaryMetric };
  strategies: string[]; statuses: string[]; exit_reasons: string[];
