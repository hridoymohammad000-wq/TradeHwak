...
class StrategyService:
    ...
    _HIGHER_INTERVAL: dict[TradingMode, str] = {
        TradingMode.SCALPING: "60",
        TradingMode.INTRADAY: "240",
    }
    ...
    def evaluate_symbol(
        ...
        candles = self._load_candles(symbol, interval, limit=120)
        if len(candles) < 60:
            return None
        ...
        if ema20 is None or ema50 is None or rsi is None or atr is None or current_price <= 0:
            return None
        ...
        overextended_pct = abs(current_price - ema20) / current_price * 100
        max_extension = max(atr_pct * 2.2, 0.8 if mode == TradingMode.SCALPING else 1.25)
        if overextended_pct > max_extension:
            return None
        ...
        extension_penalty = self._extension_penalty(overextended_pct, atr_pct)
    ...
    def _base_score(direction: Direction, trend_gap_pct: float, rsi: float) -> float:
        ...
        score += max(0.0, min(momentum, 15.0)) * 0.8
        return score

    def _higher_timeframe_score(self, symbol: str, mode: TradingMode, direction: Direction) -> float:
        try:
            candles = self._load_candles(symbol, self._HIGHER_INTERVAL[mode], limit=80)
            closes = [item.close for item in candles]
            ema20 = self._ema(closes, 20)
            ema50 = self._ema(closes, 50)
    ...
    def _extension_penalty(extension_pct: float, atr_pct: float) -> float:
        if atr_pct <= 0:
            return 0.0
        atr_multiple = extension_pct / atr_pct
        if atr_multiple > 1.8:
            return 10.0
        if atr_multiple > 1.2:
            return 5.0
